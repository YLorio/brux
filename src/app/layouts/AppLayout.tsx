...
/** Items del sidebar de desktop. En móvil estos NO se muestran — la
 *  navegación principal vive en `BottomNav` (Inicio · Movimientos · FAB
 *  Enviar · Cartera · Más). Admin y API quedan dentro de `MoreSheet`. */
const NAV = [
  ...
  return (
    <aside className="sticky top-0 hidden h-dvh w-[260px] shrink-0 border-r border-gray-200/80 bg-white dark:border-gray-800/80 dark:bg-surface-dark lg:block">
      <div className="flex h-full flex-col gap-1 p-4">
        <NavLink to="/dashboard" aria-label="Inicio" className="flex justify-center px-2 py-3">
          <Logo height={28} />
        </NavLink>
  ...
  return (
    <div className="flex min-h-dvh bg-surface-subtle dark:bg-surface-dark">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <Header />
        {/* `pb-24` deja espacio para el BottomNav (h-16 + safe area + FAB).
            En lg ya no hay nav inferior, así que volvemos a padding normal. */}
        <main className="container-responsive flex-1 py-6 pb-24 sm:py-8 lg:pb-8">
          <Outlet />
        </main>
      </div>
